...
def test_mapped_write_uses_stable_id_for_epub_and_cbz(filename):
    ...
    locator = LocatorResult(percentage=1.0, page=59 if filename == OLD_CBZ else None)
    request = UpdateProgressRequest(locator_result=locator)

    with patch(
        "src.sync_clients.booklore_sync_client.count_cbz_pages", return_value=59
    ):
        result = sync.update_progress(mapped_book(filename), request)

    assert result.success is True
    api.update_progress_by_book_id.assert_called_once_with("4", 1.0, locator)
